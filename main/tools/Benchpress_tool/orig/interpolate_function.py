import pandas as pd
import numpy as np
import os
from scipy.interpolate import interp1d
import argparse

def interpolate_landmarks(input_file):
    df = pd.read_csv(input_file, header=None, names=["frame", "landmark", "x", "y", "z"])
    df.replace('no detection', np.nan, inplace=True)
    df[['landmark', 'x', 'y', 'z']] = df[['landmark', 'x', 'y', 'z']].apply(pd.to_numeric, errors='coerce')

    all_frames = df['frame'].unique()
    interpolated_data = []

    for frame in all_frames:
        current_frame_data = df[df['frame'] == frame]

        if current_frame_data.empty:
            continue  # 如果沒有資料則跳過

        for landmark in range(11, 25):
            landmark_data = current_frame_data[current_frame_data['landmark'] == landmark]
            if landmark_data.empty:
                # 如果當前 landmark 沒有資料，進行內插
                prev_frame_data = df[(df['frame'] < frame) & (df['landmark'] == landmark)].iloc[-1] if not df[(df['frame'] < frame) & (df['landmark'] == landmark)].empty else None
                next_frame_data = df[(df['frame'] > frame) & (df['landmark'] == landmark)].iloc[0] if not df[(df['frame'] > frame) & (df['landmark'] == landmark)].empty else None
                
                interp_x = (prev_frame_data['x'] + next_frame_data['x']) / 2 if prev_frame_data is not None and next_frame_data is not None else np.nan
                interp_y = (prev_frame_data['y'] + next_frame_data['y']) / 2 if prev_frame_data is not None and next_frame_data is not None else np.nan
                interp_z = (prev_frame_data['z'] + next_frame_data['z']) / 2 if prev_frame_data is not None and next_frame_data is not None else np.nan
                
                interpolated_data.append([frame, landmark, interp_x, interp_y, interp_z])
            else:
                landmark_row = landmark_data.iloc[0]
                interpolated_data.append([frame, landmark_row['landmark'], landmark_row['x'], landmark_row['y'], landmark_row['z']])

    return pd.DataFrame(interpolated_data, columns=["frame", "landmark", "x", "y", "z"]).fillna(0)

def load_yolo_data(filename):
    data = []
    with open(filename, 'r') as f:
        for line in f:
            try:
                values = [float(x) for x in line.strip().split(',')]
                if len(values) == 5:  # YOLO 的資料應有 5 個欄位
                    data.append(values)
            except ValueError:
                print(f"Skipping line: {line}")
    return np.array(data)

def interpolate_missing_detections(yolo_data):
    frames = yolo_data[:, 0]
    x_center = yolo_data[:, 1]
    y_center = yolo_data[:, 2]
    widths = yolo_data[:, 3]
    heights = yolo_data[:, 4]

    # 找到有效檢測的幀索引
    valid_indices = np.where((x_center != -1) & (y_center != -1) & (widths != -1) & (heights != -1))[0]

    # 確保所有幀都有值
    all_frames = np.arange(int(frames.min()), int(frames.max()) + 1)

    # 使用有效檢測的幀進行插值
    interp_x_center = interp1d(frames[valid_indices], x_center[valid_indices], kind='linear', fill_value="extrapolate")
    interp_y_center = interp1d(frames[valid_indices], y_center[valid_indices], kind='linear', fill_value="extrapolate")
    interp_widths = interp1d(frames[valid_indices], widths[valid_indices], kind='linear', fill_value="extrapolate")
    interp_heights = interp1d(frames[valid_indices], heights[valid_indices], kind='linear', fill_value="extrapolate")

    # 對於所有幀進行插值
    interpolated_x_center = interp_x_center(all_frames)
    interpolated_y_center = interp_y_center(all_frames)
    interpolated_widths = interp_widths(all_frames)
    interpolated_heights = interp_heights(all_frames)

    # 返回包含所有幀的數據，確保 NaN 變成 0
    return np.nan_to_num(np.column_stack((all_frames, interpolated_x_center, interpolated_y_center, interpolated_widths, interpolated_heights)))

def interpolate_mediapipe_to_yolo(yolo_frames, mediapipe_data):
    interpolated_data = []
    landmarks = np.unique(mediapipe_data[:, 1])
    
    for landmark in landmarks:
        group = mediapipe_data[mediapipe_data[:, 1] == landmark]
        frames = group[:, 0]
        x_coords = group[:, 2]
        y_coords = group[:, 3]
        z_coords = group[:, 4]

        aligned_frames = np.linspace(yolo_frames[0], yolo_frames[-1], len(frames))
        interp_x = interp1d(aligned_frames, x_coords, kind='linear', fill_value="extrapolate")
        interp_y = interp1d(aligned_frames, y_coords, kind='linear', fill_value="extrapolate")
        interp_z = interp1d(aligned_frames, z_coords, kind='linear', fill_value="extrapolate")
        
        interpolated_x = interp_x(yolo_frames)
        interpolated_y = interp_y(yolo_frames)
        interpolated_z = interp_z(yolo_frames)
        
        for i, frame in enumerate(yolo_frames):
            interpolated_data.append([frame, landmark, interpolated_x[i], interpolated_y[i], interpolated_z[i]])
    
    return np.nan_to_num(interpolated_data)

parser = argparse.ArgumentParser()
parser.add_argument('dir', type=str)
args = parser.parse_args()
dir = args.dir
# 主流程
# 1. 內插 MediaPipe 資料
mediapipe_data = interpolate_landmarks(os.path.join(dir, 'yolo_skeleton.txt'))

# 2. 內插 YOLO 資料
yolo_data = load_yolo_data(os.path.join(dir, 'yolo_coordinates.txt'))
interpolated_yolo_data = interpolate_missing_detections(yolo_data)

# 3. 對齊 MediaPipe 資料到 YOLO 的幀數
yolo_frames = interpolated_yolo_data[:, 0]
interpolated_data_1 = interpolate_mediapipe_to_yolo(yolo_frames, mediapipe_data.values)

# 4. 儲存結果到檔案
np.savetxt(os.path.join(dir, 'yolo_skeleton_interpolated.txt'), mediapipe_data, delimiter=',', fmt='%d,%d,%.8f,%.8f,%.8f')
np.savetxt(os.path.join(dir, 'yolo_coordinates_interpolated.txt'), interpolated_yolo_data, delimiter=',', fmt='%d,%.8f,%.8f,%.8f,%.8f')
np.savetxt(os.path.join(dir, 'interpolated_mediapipe_landmarks_1.txt'), interpolated_data_1, delimiter=',', fmt='%d,%d,%.8f,%.8f,%.8f')

print("所有內插已完成並儲存。")
